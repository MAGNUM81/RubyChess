...
class Board
  ...
  class BoardPlaceError < BoardError
    def message
  ...
  class BoardRemoveError < BoardError
    def message
  ...
  def validate_positions(*positions)
    positions.each do |pos|
      false unless validate_position_in_bounds(pos)
    end
    true
  end

  def validate_position_in_bounds(position)
    (position.x >= lower_bound_x && position.x <= upper_bound_x) \
    && (position.y >= lower_bound_y && position.y <= lower_bound_y)
  end
  ...
  def place(piece, position)
    raise BoardPlaceError if position_occupied(position)

    @occupied_space[position.x][position.y][:piece] = piece
  end

  def remove(position)
    raise BoardRemoveError unless position_occupied(position)
