...
class PatternValidator
  ...
  def self.validate_move_in_bounds(from_pos, to_pos)
    @board.validate_positions(from_pos, to_pos)
  end

  def self.validate_piece_move(piece, from_pos, to_pos)
    ...
    false if abs_relative_vector.x > max_x
    false if abs_relative_vector.y > max_y
  ...
  def self.validate_pattern(pattern, from_pos, to_pos, max_x, max_y)
    ...
    pattern.each_char do |c|
      ...
      when 'x' then valid = validate_diagonal_cross(abs_relative_vector)
      when 'l' then valid = validate_knight_pattern(abs_relative_vector, max_x, max_y)
      when 'i' then valid = validate_pawn_pattern(abs_relative_vector)
